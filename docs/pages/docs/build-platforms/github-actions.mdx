...
If you have 'Require pull request reviews before merging' you will not be able to use the `GITHUB_TOKEN` available in the action.
This token does not have access to toggling these settings.
You _must_ create a personal access token (PAT) with `repo` access.

In the following example we have added a PAT as `GH_TOKEN` and use that with the GitHub API.
